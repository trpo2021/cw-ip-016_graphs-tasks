...
int nmain(int argv, char *arg[]) {
  ifstream inp;
  ofstream out;
  int nodeBeging = -1, nodeEnd = -1;
  int mode = 2;
  nodeBeging = -1;
  nodeEnd = -1;
  for (int i = 1; i < argv; ++i) {
    ...
    try {
      if (t == "-i") {
        i++;
        if (i >= argv)
          throw string("no argument for -i");
        string tt = arg[i];
        ...
        i++;
        if (i >= argv)
          throw string("no argument for -o");
        string tt = arg[i];
        ...
        ++i;
        if (i >= argv)
          throw string("no argument for -m");
        string tt = arg[i];
        if (tt == "sp") {
          mode = 0;
        } else if (tt == "lp") {
          ...
          throw "invalid argument for -m, expected [sp, lp, cp], found " + tt;
        continue;
      }
  ...
  if (nodeBeging == -1 || nodeEnd == -1) {
    cout << "Begin node or end node is not declarated\n";
    return 0;
  }
  if (inp.is_open())
    cin.rdbuf(inp.rdbuf());
...
#ifndef TEST
int main(int argv, char *arg[]) { return nmain(argv, arg); }
#endif
